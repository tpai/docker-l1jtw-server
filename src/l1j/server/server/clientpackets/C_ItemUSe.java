...
public class C_ItemUSe extends ClientBasePacket {
	...
	public C_ItemUSe(byte abyte0[], ClientThread client) throws Exception {
		...
		if ((itemId == 40088) || (itemId == 40096) || (itemId == 140088)) {
			s = readS();
				...
				) || (itemId == 40954) || (itemId == 40955)
				|| (itemId == 40956) || (itemId == 40957 // 加工された火ダイア
		...
		if (pc.getCurrentHp() > 0) {
			...
			if ((itemId == 40077)
				...
				if ((weaponId == 36) || (weaponId == 183)
						|| ((weaponId >= 250) && (weaponId <= 255))) { // イリュージョン武器
				...
				if ((armorId == 20161)
						|| ((armorId >= 21035) && (armorId <= 21038))) { // イリュージョン防具
				...
				// レッドポーション、濃縮体力回復剤、象牙の塔の体力回復剤
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (((itemId > 40169) && (itemId < 40226))
						|| ((itemId >= 45000) && (itemId <= 45022))) { // 魔法書
					...
					if (l1iteminstance1.getItem().getType2() != 2) {
						...
						return;
							...
							&& (l1iteminstance1.getItem().getType() == 8
									...
									|| l1iteminstance1.getItem().getType() == 10 || l1iteminstance1
									.getItem().getType() == 12)) {
						...
						case 9:
							...
							if (chance < 25) {
								l1iteminstance1.setaddHp(l1iteminstance1
										.getaddHp() + 2);
								...
								l1iteminstance1.setaddMp(l1iteminstance1
										.getaddMp() + 1);
								if (l1iteminstance1.isEquipped()) {
									pc.addMaxMp(1);
									// pc.addAc(-1);
						...
						case 5:
							if (chance < 25) {
								l1iteminstance1.setaddHp(l1iteminstance1
										.getaddHp() + 2);
								l1iteminstance1.setMpr(l1iteminstance1
										.getItem().get_addmpr() + 1);
								if (l1iteminstance1.isEquipped()) {
									pc.addMaxHp(2);
									// pc.addAc(-1);
								}
								...
								l1iteminstance1.setaddMp(l1iteminstance1
										.getaddMp() + 1);
								l1iteminstance1.setaddSp(l1iteminstance1
										.getItem().get_addsp() + 1);
								if (l1iteminstance1.isEquipped()) {
									pc.addMaxMp(1);
									// pc.addAc(-1);
			...
			if (!item.isIdentified() || (item.getEnchantLevel() == 0)) {
				...
				case -1:
					s = (new StringBuilder()).append(pm)
							.append(item.getEnchantLevel()).append(" ")
		...
		if (item.getItem().getType2() == 2) {
			if (item.isEquipped()) {
				...
				if ((i2 == 20011) || (i2 == 20110) || (i2 == 21108)
						|| (i2 == 120011)) { // マジックヘルム、マジックチェーンメイル、キャラクター名の魔法抵抗のＴシャツ
					pc.addMr(i);
	...
	private void FailureEnchant(L1PcInstance pc, L1ItemInstance item,
		...
		if (itemType == 1) { // 武器
			if (!item.isIdentified() || (item.getEnchantLevel() == 0)) {
				...
				s = (new StringBuilder()).append(pm)
						.append(item.getEnchantLevel()).append(" ")
						.append(nameId).toString(); // \f1%0が強烈に%1光ったあと、蒸発してなくなります。
				sa = "$245";
			}
	...
	private void SpellBook1(L1PcInstance pc, L1ItemInstance l1iteminstance,
		...
		for (int j6 = 97; j6 < 112; j6++) {
			...
			if (Config.CLIENT_LANGUAGE == 3) {
				...
				s1 = (new StringBuilder()).append("闇精霊の水晶(")
						.append(l1skills.getName()).append(")").toString();
			}
	...
	private void SpellBook2(L1PcInstance pc, L1ItemInstance l1iteminstance) {
		...
		for (int j6 = 129; j6 <= 176; j6++) {
			...
			if (Config.CLIENT_LANGUAGE == 3) {
				...
				s1 = (new StringBuilder()).append("精霊の水晶(")
						.append(l1skills.getName()).append(")").toString();
			}
			if (l1iteminstance.getItem().getName().equalsIgnoreCase(s1)) {
	...
	private void SpellBook5(L1PcInstance pc, L1ItemInstance l1iteminstance,
		...
		for (int j6 = 181; j6 <= 195; j6++) {
			...
			if (Config.CLIENT_LANGUAGE == 3) {
				...
				s1 = (new StringBuilder()).append("ドラゴンナイトの書板（")
						.append(l1skills.getName()).append("）").toString();
			}
			if (l1iteminstance.getItem().getName().equalsIgnoreCase(s1)) {
				int l6 = l1skills.getSkillLevel();
	...
	private void SpellBook6(L1PcInstance pc, L1ItemInstance l1iteminstance,
		...
		for (int j6 = 201; j6 <= 220; j6++) {
			...
			if (Config.CLIENT_LANGUAGE == 3) {
				...
				s1 = (new StringBuilder()).append("記憶の水晶（")
						.append(l1skills.getName()).append("）").toString();
			}
			if (l1iteminstance.getItem().getName().equalsIgnoreCase(s1)) {
				int l6 = l1skills.getSkillLevel();
	...
	private void polyAction(L1PcInstance attacker, L1Character cha) {
		...
		if (cha instanceof L1PcInstance) {
			...
			if (mob.getLevel() < 50) {
				...
				if ((npcId != 45338)
						&& (npcId != 45370)
						&& (npcId != 45456 // クロコダイル、バンディットボス、ネクロマンサー
						) && (npcId != 45464)
						&& (npcId != 45473)
						&& (npcId != 45488 // セマ、バルタザール、カスパー
						) && (npcId != 45497) && (npcId != 45516)
						&& (npcId != 45529 // メルキオール、イフリート、ドレイク(DV)
						) && (npcId != 45458)) { // ドレイク(船長)
					L1Skills skillTemp = SkillsTable.getInstance().getTemplate(
							SHAPE_CHANGE);
	...
	private void useToiTeleportAmulet(L1PcInstance pc, int itemId,
		...
		if ((itemId == 40289) || (itemId == 40293)) { // 11,51Famulet
			...
			if ((pc.getX() >= 32815) && (pc.getX() <= 32820)
					&& (pc.getY() >= 32815) && (pc.getY() <= 32820)
					&& (pc.getMapId() == 101)) {
				isTeleport = true;
			}
		} else if ((itemId == 40291) || (itemId == 40295)) { // 31,71Famulet
			if ((pc.getX() >= 32779) && (pc.getX() <= 32784)
					&& (pc.getY() >= 32778) && (pc.getY() <= 32783)
					&& (pc.getMapId() == 101)) {
				isTeleport = true;
			}
			...
			if ((pc.getX() >= 32706) && (pc.getX() <= 32710)
					&& (pc.getY() >= 32909) && (pc.getY() <= 32913)
					&& (pc.getMapId() == 190)) {
				isTeleport = true;
			}
		}
